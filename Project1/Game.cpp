...
std::string Game::takePlayerTurn()
{
    for (;;)
    {
        ...
        else if (playerMove.size() == 1)
        {
            ...
            {
                m_arena->history().display(); // displays the grid by clearing the screen of the arena grid
                std::cout << "Press enter to continue.";
                std::cin.ignore(10000, '\n');
                m_arena->display(""); // displays the arena grid by clearing the screen of the history grid
            }
            else if (decodeDirection(playerMove[0], dir))
            {
                ...
                std::cout << "Player move must be nothing, or 1 character n/e/s/w/x/h." << std::endl; // put this statement into an else, so it doesnt cout every single time
                                                                                                // when h is pressed (since h is a valid key to press now)
        }
    }
}
